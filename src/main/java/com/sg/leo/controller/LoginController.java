	...
	public @ResponseBody ResponseDTO<?> login(@RequestBody User user, HttpSession session) {
		...
		if (findUser.getUsername() != null) {
			if (user.getPassword().equals(findUser.getPassword())) {
				...
				return new ResponseDTO<>(HttpStatus.OK.value(), user.getUsername() + "님 로그인성공");
			} else {
				return new ResponseDTO<>(HttpStatus.BAD_REQUEST.value(), user.getPassword() + "와 비밀번호다름");
			}

		} else {
			return new ResponseDTO<>(HttpStatus.BAD_REQUEST.value(), user.getUsername() + "님 아이디없음, 회원가입바람");
